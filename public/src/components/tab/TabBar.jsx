...
export class TabBar extends Component {
    ...
    return (
        <div className="tab mx-5 my-4 px-4 border-bottom border-dark">
            <div className='user'><span>{this.props.user.email}</span><span> {this.props.user.login}</span>
            </div>
            <a className='main-link' href={'/'}>{translate('userPages')}</a>
            <div className='left language'>
                <select className='form-control h-100 select' value={this.state.language} onChange={this.changeLanguage}>
                <option value='en'>en</option>
                <option value='ua'>ua</option>
                </select>
                <a className='btn btn-primary' href='/' onClick={Logout }>{translate('logout')}</a>
             </div>
        </div>
    );
  };

...
export class login extends Component {
    ...
    handleSubmit(event) {
        ...
        this.state.password.length > 0 ? ErrorPassword = false : ErrorPassword = true;
            ...
            .then((data) => {
                try {  
                    if (data) {
                        ...
                        this.props.setUserData(data.userId, data.email, data.login, data.role)
                   ...
                   <div className="translate-2 my-3">
                     <span>{translate('login')}</span>
                        <input
                            ...
                            />
                            {ErrorLogin && (<span className='text-danger'>{translate('error_RequireLogin')}</span>)}
                        </div>
                   <div className="translate-2 my-3">
                      <span>{translate('password')}</span>
                        <input
                            ...
                            className="form-control"
                            />
                            {ErrorPassword && (<span className='text-danger'>{translate('error_RequirePassword')}</span>)}
                            {ErrorLog && (<span className='text-danger'>{translate('error_login')}</span>)}
                   </div>
                   <input type="submit" className="btn btn-success my-3 w-100" value={translate('log_in')} />
                 </form>
                </div>
            </div>
        );
